...
with tab1:
    ...
    with col2:
        ...
        if predict_btn and st.session_state.models:
            ...
            calcium_hardness_prediction = None
            total_hardness_prediction = None
            ...
            for target, model_data in st.session_state.models.items():
                ...
                if "Total" in target:
                    total_hardness_prediction = prediction
                else:
                    calcium_hardness_prediction = prediction
            
            # Display Total Hardness
            with cols[0]:
                if total_hardness_prediction is not None:
                    st.metric(label="**Total Hardness**", value=f"{total_hardness_prediction:.0f} ppm")
                    config = st.session_state.best_configs.get('Total_Hardness_ppm', {})
                    st.caption(f"🤖 {config.get('algorithm', 'N/A')}")
                    st.caption(f"📊 R² = {config.get('r2', 0):.3f}")
            
            # Display Calculated Hardness (Calcium Hardness)
            with cols[1]:
                if calcium_hardness_prediction is not None:
                    st.metric(label="**Calculated Hardness (Ca)**", value=f"{calcium_hardness_prediction:.0f} ppm")
                    config = st.session_state.best_configs.get('Calculated_Hardness_ppm', {})
                    st.caption(f"🤖 {config.get('algorithm', 'N/A')}")
                    st.caption(f"📊 R² = {config.get('r2', 0):.3f}")
            
            # Display Calcium (ppm) = Calculated Hardness / 2.5
            with cols[2]:
                if calcium_hardness_prediction is not None:
                    calcium_ppm = calcium_hardness_prediction / 2.5
                    st.metric(label="**Calcium (Ca²⁺)**", value=f"{calcium_ppm:.0f} ppm")
                    st.caption("📐 Formula:")
                    st.caption("Ca = Calc. Hardness / 2.5")
